...
set "INPUT_DIR=%PROJECT_DIR%\comfyui\input"
set "OUTPUT_DIR=%PROJECT_DIR%\comfyui\output"
set "SNAPSHOT_DIR=%PROJECT_DIR%\comfyui\snapshots"
set "CONFIG_FILE=%CONFIG_DIR%\config.json"

:: Convert backslashes to forward slashes (for Docker Compose)
set "PATCH_PATH_DOCKER=!PATCH_DIR:\=/!"
set "INPUT_PATH_DOCKER=!INPUT_DIR:\=/!"
set "OUTPUT_PATH_DOCKER=!OUTPUT_DIR:\=/!"
set "SNAPSHOT_PATH_DOCKER=!SNAPSHOT_DIR:\=/!"
...
echo # Models are stored in Docker named volume (comfyui_models)>> "%ENV_FILE%"
echo INPUT_PATH=!INPUT_PATH_DOCKER!>> "%ENV_FILE%"
echo OUTPUT_PATH=!OUTPUT_PATH_DOCKER!>> "%ENV_FILE%"
echo SNAPSHOT_PATH=!SNAPSHOT_PATH_DOCKER!>> "%ENV_FILE%"
...
set "USER_NODES_DIR=%PROJECT_DIR%\comfyui\user_nodes"
...
if not exist "%USER_NODES_DIR%\ComfyUI-Manager" (
    echo.
    echo [CLONE] ComfyUI-Manager pinned to %MANAGER_COMMIT%...
    git clone https://github.com/ltdrdata/ComfyUI-Manager.git "%USER_NODES_DIR%\ComfyUI-Manager"
    if !errorlevel! equ 0 (
        git -C "%USER_NODES_DIR%\ComfyUI-Manager" checkout %MANAGER_COMMIT%
    ) else (
        echo [X] Failed to clone ComfyUI-Manager.
    )
)

if not exist "%USER_NODES_DIR%\ComfyUI-Lora-Manager" (
    echo.
    echo [CLONE] ComfyUI-Lora-Manager pinned to %LORA_MANAGER_COMMIT%...
    git clone https://github.com/willmiao/ComfyUI-Lora-Manager.git "%USER_NODES_DIR%\ComfyUI-Lora-Manager"
    if !errorlevel! equ 0 (
        git -C "%USER_NODES_DIR%\ComfyUI-Lora-Manager" checkout %LORA_MANAGER_COMMIT%
    ) else (
        echo [X] Failed to clone ComfyUI-Lora-Manager.
    )
)
...
if exist "%WSLCONFIG_SRC%" (
    if not exist "%WSLCONFIG_DST%" (
        ...
        echo [OK] WSL2 memory limit applied - 26GB. Takes effect on Docker restart.
    ) else (
...
echo ====================================================
echo.
echo   1. ComfyUI (port 8188^)
echo   2. Hooking Server (port 8189^)
echo   3. All
echo   0. Back
echo.
set "LOG_CHOICE="
...
if "%LOG_CHOICE%"=="1" (
    start "ComfyUI Log" cmd /k chcp 65001 ^>nul 2^>^&1 ^& echo === ComfyUI Log ^(Ctrl+C to close^) === ^& echo. ^& docker compose -f "%PROJECT_DIR%\docker-compose.yml" logs -f --tail=100 comfyui
    goto menu
)
if "%LOG_CHOICE%"=="2" (
    start "Hooking Server Log" cmd /k chcp 65001 ^>nul 2^>^&1 ^& echo === Hooking Server Log ^(Ctrl+C to close^) === ^& echo. ^& docker compose -f "%PROJECT_DIR%\docker-compose.yml" logs -f --tail=100 hooking_server
    goto menu
)
if "%LOG_CHOICE%"=="3" (
    start "ComfyPack Logs (All)" cmd /k chcp 65001 ^>nul 2^>^&1 ^& echo === ComfyPack Logs ^(Ctrl+C to close^) === ^& echo. ^& docker compose -f "%PROJECT_DIR%\docker-compose.yml" logs -f --tail=100
    goto menu
)
goto menu
...
echo     Models (Docker volume), patch data, and config are safe.
echo.
set "CONFIRM_RM="
...
echo     Models (Docker volume), patch data, and config will be kept.
echo     Only images are removed.
echo.
set "CONFIRM="
